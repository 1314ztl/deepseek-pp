/**
 * Userscript port of core/memory/codec.ts.
 *
 * Validation is intentionally strict: an undecodable record throws instead of
 * being silently defaulted, so corrupt data fails visibly rather than
 * overwriting the user's memory set (extension invariant).
 */
...
export function decodePersistedMemoryRecord(value: unknown, path = 'memory'): Memory {
  const object = objectValue(value, path);
  const id = positiveSafeInteger(object.id, `${path}.id`);
  return { ...decodeStoredMemory(object, path), id };
}

export function decodeStoredMemory(value: unknown, path = 'memory'): Omit<Memory, 'id'> {
  const object = objectValue(value, path);
...
/** Decoder for user-supplied drafts and imported files (no timestamps yet). */
export function decodeImportedMemory(value: unknown, path = 'memory'): NewMemory {
  const object = objectValue(value, path);
  ...
  return {
    ...
    tags: object.tags === undefined ? [] : stringArray(object.tags, `${path}.tags`),
    pinned: object.pinned === undefined
      ? false
      : booleanValue(object.pinned, `${path}.pinned`),
  };
}
